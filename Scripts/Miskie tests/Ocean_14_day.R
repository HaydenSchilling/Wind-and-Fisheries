...
Estuary_sp <- Misk_list %>% filter(Category == "ocean") %>% select(NIMO_NAME)
...
write.csv(A_test,"Anova Ocean 14 days 50km.csv", row.names = F)
S_test <- broom.mixed::tidy((fit4))
write.csv(S_test,"Summary Ocean 14 days 50km.csv", row.names = F)
...
write.csv(heat_dataM, "Heat data Ocean 14 day 50km.csv", row.names = F)
...
ggsave("Ocean 14 day plots_Alternate 50km.png", width = 21, height = 14.8, units = "cm", dpi = 600)
