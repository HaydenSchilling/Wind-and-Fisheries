...
# 
# Luderick <- subset(mydata, Species == "Luderick")
# 
# m1 <- lm(log(CPUE)~ Onshore_Winds + Current_Wind + X135_degree_winds, data = Luderick)
# plot(m1)
# summary(m1)
# anova(m1)
# 
...
plot(m1)
summary(m1)
anova(m1) # Luderick interaction with winds
...
pred_dat <- data.frame("X135_degree_winds.standardised" = seq(from = -2,
                       ...
                       #"Current_Wind" = mean(Luderick$Current_Wind),
                       "Drought_Months" = 4)

# # Old prediction code - does not do SE

# Pred_Luderick <- predict(m1, newdata = pred_dat, type = "response", se.fit = T)
# 
...
Pred_Luderick <- predict(m1, newdata=pred_dat,type= "response", re.form=NA, se.fit = T, nsim = 500)
Pred_Luderick

#plot(x = pred_dat$X135_degree_winds.standardised, y=Pred_Luderick$fit, type = "l",
#     ylab = "Predicted Luderick CPUE standardised", xlab = "Standardised Southeast Winds",
#     main = "Predicted coastal species abundance \nfor no drought and mean NE winds") #, ylim=c(0,1)
#lines(x = pred_dat$X135_degree_winds.standardised, y=(Pred_Luderick$fit-Pred_Luderick$se.fit), type = "l", col = "blue")
#lines(x = pred_dat$X135_degree_winds.standardised, y=(Pred_Luderick$fit+Pred_Luderick$se.fit), type = "l", col = "blue")
...
#ggsave("plots/Luderick CPUE and SE wind predictions.pdf", height = 14.8, width = 21, units = "cm")
#ggsave("plots/Luderick CPUE and SE wind predictions.png", height = 14.8, width = 21, units = "cm", dpi = 600)
