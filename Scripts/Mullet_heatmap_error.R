...
# 
# Mullet <- subset(mydata, Species == "Mullet")
# 
# m1 <- lm(log(CPUE)~ Onshore_Winds + Current_Wind + X135_degree_winds, data = Mullet)
# plot(m1)
# summary(m1)
# anova(m1)
# 
...
plot(m1)
summary(m1)
anova(m1) # Mullet interaction with winds
...
pred_dat <- data.frame("X135_degree_winds.standardised" = seq(from = -2,
                       ...
                       #"Current_Wind" = mean(Mullet$Current_Wind),
                       "Drought_Months" = 4)

# # Old prediction code - does not do SE

# Pred_Mullet <- predict(m1, newdata = pred_dat, type = "response", se.fit = T)
# 
...
Pred_Mullet <- predict(m1, newdata=pred_dat,type= "response", re.form=NA, se.fit = T, nsim = 300)
Pred_Mullet

#plot(x = pred_dat$X135_degree_winds.standardised, y=Pred_Mullet$fit, type = "l",
#     ylab = "Predicted Mullet CPUE standardised", xlab = "Standardised Southeast Winds",
#     main = "Predicted coastal species abundance \nfor no drought and mean NE winds") #, ylim=c(0,1)
#lines(x = pred_dat$X135_degree_winds.standardised, y=(Pred_Mullet$fit-Pred_Mullet$se.fit), type = "l", col = "blue")
#lines(x = pred_dat$X135_degree_winds.standardised, y=(Pred_Mullet$fit+Pred_Mullet$se.fit), type = "l", col = "blue")
